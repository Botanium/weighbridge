let port; // Declare port variable globally so it can be accessed by both connect and disconnect functions
...
frappe.ui.form.on('Loading Transaction', {
    ...
    refresh: function(frm) {
        frm.add_custom_button('Read Entry Weight', async function() {
            if ('serial' in navigator) {
                try {
                    ...
                    console.log('Reading from serial port finished');
                } catch (error) {
        ...
        frm.add_custom_button('Disconnect Serial Port', async function() {
            if (port) {
                try {
                    await port.close();
                    console.log('Serial port closed');
                    frappe.msgprint(__('Serial port disconnected.'));
                } catch (error) {
                    console.log('Error:', error);
                    frappe.msgprint(__('Error disconnecting serial port: ') + error);
                }
            } else {
                frappe.msgprint(__('No serial port is connected.'));
